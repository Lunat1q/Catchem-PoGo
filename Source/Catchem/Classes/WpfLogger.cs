...
namespace Catchem.Classes
{
    public class WpfLogger : ILogger
    {
        ...
        private static string _strTransferred = "TRANSFERRED";
        ...
        private static string _strLureMod = "LUREMOD";
        private static string _strSuccess = "SUCCESS";
        ...
        public void SetSession(ISession session)
        {
            ...
            if (_session != null)
            {
                ...
                _strTransferred = _session.Translation.GetTranslation(PoGo.PokeMobBot.Logic.Common.TranslationString.LogEntryTransfered);
        ...
        public void Write(string message, LogLevel level = LogLevel.Info, ConsoleColor color = ConsoleColor.Black, ISession session = null)
        {
            ...
            {
                case LogLevel.Error:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strError}) {message}", Color.FromRgb(255, 0, 0));
                    break;
                case LogLevel.Warning:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strAttention}) {message}", Color.FromRgb(254, 229, 5));
                    break;
                case LogLevel.Info:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strInfo}) {message}", Color.FromRgb(239, 239, 239));
                    break;
                case LogLevel.Pokestop:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strPokestop}) {message}", Color.FromRgb(0, 190, 255));
                    break;
                case LogLevel.Farming:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strFarming}) {message}", Color.FromRgb(157, 255, 0));
                    break;
                case LogLevel.Recycling:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strRecycling}) {message}", Color.FromRgb(255, 106, 240));
                    break;
                case LogLevel.Caught:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strPkmn}) {message}", Color.FromRgb(8, 206, 8));
                    break;
                case LogLevel.Transfer:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strTransferred}) {message}", Color.FromRgb(0, 255, 214));
                    break;
                case LogLevel.Evolve:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strEvolved}) {message}", Color.FromRgb(255, 230, 0));
                    break;
                case LogLevel.Berry:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strBerry}) {message}", Color.FromRgb(255, 0, 194));
                    break;
                case LogLevel.Egg:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strEgg}) {message}", Color.FromRgb(167, 249, 255));
                    break;
                case LogLevel.Debug:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strDebug}) {message}", Colors.White);
                    break;
                case LogLevel.Update:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strUpdate}) {message}", Color.FromRgb(145, 255, 0));
                    break;
                case LogLevel.None:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strNone}) {message}", Colors.White);
                    break;
                case LogLevel.Escape:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strEscape}) {message}", Color.FromRgb(255, 177, 0));
                    break;
                case LogLevel.Flee:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strFlee}) {message}", Color.FromRgb(158, 255, 255));
                    break;
                case LogLevel.Favorite:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strFavourite}) {message}", Color.FromRgb(255, 0, 159));
                    break;
                case LogLevel.UnFavorite:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strUnFavourite}) {message}", Color.FromRgb(255, 0, 159));
                    break;
                case LogLevel.Gym:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strGym}) {message}", Color.FromRgb(192, 0, 255));
                    break;
				case LogLevel.Telegram:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strTelegram}) {message}", Color.FromRgb(60, 197, 255));
                    break;
                case LogLevel.LureModule:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strLureMod}) {message}", Colors.HotPink);
                    break;
                case LogLevel.Success:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strSuccess}) {message}", Colors.LawnGreen);
                    break;
                default:
                    SendWindowMsg(MainRpc.Log, session, $"[{DateTime.Now.ToString("HH:mm:ss")}] ({_strError}) {message}", Color.FromRgb(255, 255, 255));
                    break;
            }
        }

        public void SendWindowMsg(MainRpc msgType, ISession session, params object[] objData)
        {
