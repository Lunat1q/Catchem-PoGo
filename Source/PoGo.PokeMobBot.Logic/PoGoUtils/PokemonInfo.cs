    ...
    public static class PokemonInfo
    {
        ...
        public static int CalculateMinCp(PokemonData poke)
        {
            return
                Math.Max(
                    (int)
                        Math.Floor(0.1*CalculateCpMultiplier(poke, 0, 0, 0)*
                                   Math.Pow(poke.CpMultiplier + poke.AdditionalCpMultiplier, 2)), 10);
        }

        public static double CalculateCpMultiplier(PokemonData poke, int atk, int def, int sta)
        {
            var baseStats = GetBaseStats(poke.PokemonId);
            return (baseStats.BaseAttack + atk) * Math.Sqrt(baseStats.BaseDefense + def) * Math.Sqrt(baseStats.BaseStamina + sta);
        }
        ...
        public static double[] CpMultiplier => new[]
        {
            0.094, 0.1351374, 0.1663979, 0.1926509, 0.2157325, 0.2365727, 0.2557201, 0.2735304, 0.2902499,
            0.3060574, 0.3210876, 0.335445, 0.3492127, 0.3624578, 0.3752356, 0.3875924, 0.3995673, 0.4111936,
            0.4225, 0.4335117, 0.4431076, 0.45306, 0.4627984, 0.4723361, 0.481685, 0.4908558, 0.4998584,
            0.5087018, 0.517394, 0.5259425, 0.5343543, 0.5426358, 0.5507927, 0.5588306, 0.5667545, 0.5745692,
            0.5822789, 0.5898879, 0.5974, 0.6048188, 0.6121573, 0.6194041, 0.6265671, 0.6336492, 0.640653,
            0.647581, 0.6544356, 0.6612193, 0.667934, 0.6745819, 0.6811649, 0.6876849, 0.6941437, 0.7005429,
            0.7068842, 0.7131691, 0.7193991, 0.7255756, 0.7317, 0.734741, 0.7377695, 0.7407856, 0.7437894,
            0.7467812, 0.749761, 0.7527291, 0.7556855, 0.7586304, 0.7615638, 0.7644861, 0.7673972, 0.7702973,
            0.7731865, 0.776065, 0.7789328, 0.7817901, 0.784637, 0.7874736, 0.7903, 0.7931164
        };
        ...
        public static double GetLevel(this PokemonData poke)
        {
            ...
            var i = 0;
            while (cpm > CpMultiplier[i] && Math.Abs(cpm - CpMultiplier[i]) > 0.001)
            {
                i++;
            }
        ...
        private static int FindTheClosest(IReadOnlyList<double> array, double val)
        {
            var delta = double.MaxValue;
            var indx = 0;
            for (var i = 0; i < array.Count; i++)
            {
                if ((Math.Abs(val - array[i]) > delta)) continue;
                delta = Math.Abs(val - array[i]);
                indx = i;
            }
            return indx;
        }
        ...
        public static double PossibleCpAfterEvolution(this PokemonId poke, int ivAtk, int ivDef, int ivStamina, double cpm)
        {
            var evoBase = GetBaseStats(poke);
            var maxAttackNoCpM = (evoBase.BaseAttack + ivAtk);
            var maxDefenseNoCpM = (evoBase.BaseDefense + ivDef);
            var maxStaminaNoCpM = (evoBase.BaseStamina + ivStamina);

           return Math.Ceiling(maxAttackNoCpM * Math.Sqrt(maxDefenseNoCpM) * Math.Sqrt(maxStaminaNoCpM) * Math.Pow(cpm, 2) / 10) - 1;
        }

        public static double GetMaxCpAtTrainerLevel(PokemonData poke, int trainerLevel)
        {
            var pokemonMaxLevel = Math.Min(trainerLevel + 1.5, 40);
            var cpm = CpMultiplier[Math.Min((int)(pokemonMaxLevel * 2 - 2), CpMultiplier.Length - 1)];
