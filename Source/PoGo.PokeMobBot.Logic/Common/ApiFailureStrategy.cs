...
namespace PoGo.PokeMobBot.Logic.Common
{
    public class ApiFailureStrategy : IApiFailureStrategy
    {
        ...
        private async void DoLogin()
        {
            ...
            {
                Logger.Write("APIFailureStrategy: " + ex.Message);
            }
        }
        ...
        public async Task<ApiOperation> HandleApiFailure(RequestEnvelope request, ResponseEnvelope response)
        {
            ...
            if (_retryCount % 5 == 0)
            {
                ...
                catch (Exception ex) when (ex is InvalidResponseException || ex is TaskCanceledException)
                {
                    ...
                    if (_session.Profile != null && _session.Profile.Banned)
                    {
                        _session.EventDispatcher.Send(new ErrorEvent
                        {
                            Message = _session.Translation.GetTranslation(TranslationString.NianticPlayerBanned)
                        });
                    }
                    else if (_session.Profile != null && _session.Profile.Warn)
                    {
                        _session.EventDispatcher.Send(new ErrorEvent
                        {
                            Message = _session.Translation.GetTranslation(TranslationString.NianticPlayerWarn)
                        });
                    }
