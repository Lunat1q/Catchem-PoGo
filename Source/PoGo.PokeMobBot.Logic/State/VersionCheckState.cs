...
namespace PoGo.PokeMobBot.Logic.State
{
    public class VersionCheckState : IState
    {
        public const string VersionUri =
            "https://raw.githubusercontent.com/Lunat1q/Catchem-PoGo/master/latest.version";
        ...
        public async Task<IState> Execute(ISession session, CancellationToken cancellationToken)
        {
            ...
            var APIversionValid = await ApiVersionCheckState.Execute(session, cancellationToken);
        ...
        public bool IsLatest(ISession session)
        {
            try
            {
                var regex = new Regex(@"(\d{1,})\.(\d{1,})\.(\d{1,})\.(\d{1,})");
