...
namespace PoGo.PokeMobBot.Logic.Utils
{
    public static class RoutingUtils
    {
        ...
        public static IEnumerable<GeoCoordinate> DecodeWAltToList(string polyline, int precision = 5)
        {
            if (string.IsNullOrEmpty(polyline))
                throw new ArgumentNullException(nameof(polyline));
        ...
        public static string EncodeWAltToPolyline(IEnumerable<GeoCoordinate> points)
        {
            var encodedRoute = new StringBuilder();
            ...
            var encodeDiff = (Action<int>)(diff =>
            {
                ...
                while (rem >= 0x20)
                {
                    encodedRoute.Append((char)((0x20 | (rem & 0x1f)) + 63));

                    rem >>= 5;
                }

                encodedRoute.Append((char)(rem + 63));
            });
            ...
            foreach (var point in points)
            {
                ...
                int alt = (int)Math.Round(point.Altitude * 1E5);

                encodeDiff(lat - lastLat);
                encodeDiff(lng - lastLng);
                encodeDiff(alt - lastAlt);
        ...
        public static string EncodeToPolyline(IEnumerable<GeoCoordinate> points)
        {
            var encodedRoute = new StringBuilder();
